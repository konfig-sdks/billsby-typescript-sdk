...
import { readableStreamToString, BillsbyError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new BillsbyError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new BillsbyError(e, e.response?.data)
                }
                if (innerError instanceof BillsbyError) {
                    // Got "BillsbyError" from the above try block
                    throw innerError;
                }
